	...
	Request({ // key is not referral but API-key: https://api.yandex.com/translate/doc/dg/concepts/api-overview.xml
		...
		onComplete: function (response) {
			...
			if (input == translated && wasTranslatedSecondTime == false) {  // if input on Russian and we receive the same text -
				...
				if (translated.length > 75) popup(translated);
				else {
...
function popup(text) {
	components.classes['@mozilla.org/alerts-service;1'].getService(components.interfaces.nsIAlertsService).showAlertNotification(null, 'translate.yandex.ru', translated, false, '', null);
}
